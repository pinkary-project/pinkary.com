...
it('can edit a post and change its channel', function (): void {
    ...
    $question = Question::factory()->for($channelA)->create([
        'from_id' => $user->id,
        'to_id' => $user->id,
        'content' => '__UPDATE__',
...
it('prevents unverified users from creating channels via store', function (): void {
    $user = User::factory()->unverified()->create();

    Livewire::actingAs($user)
        ->test(Create::class, ['toId' => $user->id])
        ->set('content', 'Testing post creation with channel by unverified user')
        ->set('channelName', 'New Channel')
        ->call('store')
        ->assertRedirect(route('verification.notice'));

    expect(Channel::where('name', 'New Channel')->exists())->toBeFalse()
        ->and(Question::where('answer', 'Testing post creation with channel by unverified user')->exists())->toBeFalse();
});

it('prevents unverified users from creating channels via update', function (): void {
    $user = User::factory()->unverified()->create();
    $question = Question::factory()->sharedUpdate()->create([
        'from_id' => $user->id,
        'to_id' => $user->id,
        'answer' => 'Initial update',
        'answer_created_at' => now(),
    ]);

    Livewire::actingAs($user)
        ->test(Edit::class, ['questionId' => $question->id])
        ->set('channelName', 'Another Channel')
        ->call('update')
        ->assertRedirect(route('verification.notice'));

    expect(Channel::where('name', 'Another Channel')->exists())->toBeFalse();
});
...
it('can update post channel in edit composer', function (): void {
    ...
    $question = Question::factory()->sharedUpdate()->for($channel1)->create([
        'from_id' => $user->id,
        'to_id' => $user->id,
...
it('validates direct channelName property tampering on store', function (): void {
    $user = User::factory()->create();

    Livewire::actingAs($user)
        ->test(Create::class, ['toId' => $user->id])
        ->set('content', 'Testing invalid channel tampering')
        ->set('channelName', 'Invalid @ Name!')
        ->call('store')
        ->assertHasErrors(['channelName']);

    Livewire::actingAs($user)
        ->test(Create::class, ['toId' => $user->id])
        ->set('content', 'Testing blank slug channel')
        ->set('channelName', '---')
        ->call('store')
        ->assertHasErrors(['channelName']);
});

it('validates non-existent channelId on store', function (): void {
    $user = User::factory()->create();

    Livewire::actingAs($user)
        ->test(Create::class, ['toId' => $user->id])
        ->set('content', 'Testing non existent channel')
        ->set('channelId', 999999)
        ->call('store')
        ->assertHasErrors(['channelId']);
});

it('throttles excessive createChannel calls', function (): void {
    $user = User::factory()->create();

    $component = Livewire::actingAs($user)->test(Create::class, ['toId' => $user->id]);

    for ($i = 0; $i < 10; $i++) {
        $component->call('createChannel', "Channel {$i}");
    }

    $component->call('createChannel', 'Channel 11')
        ->assertHasErrors(['newChannel']);
});

it('validates channelName tampering and non-existent channelId on update', function (): void {
    $user = User::factory()->create();
    $question = Question::factory()->sharedUpdate()->create([
        'from_id' => $user->id,
        'to_id' => $user->id,

...
@php
    $resolvedChannelId = $channelId ?? $this->channelId;
    $resolvedChannelName = $channelName ?: ($this->selectedChannel?->name ?? ($this->channelName ?? ''));
    $resolvedChannels = $channels !== null
        ? $channels
        : $this->availableChannels->map(fn ($channel) => ['id' => $channel->id, 'name' => $channel->name])->values();
@endphp
...
<div
    ...
    x-data="{
        open: false,
        search: '',
        initialChannelId: @js($resolvedChannelId),
        initialChannelName: @js($resolvedChannelName),
        selectedChannelId: @js($resolvedChannelId),
        selectedChannelName: @js($resolvedChannelName),
        channels: @js($resolvedChannels),
        searchCache: {},
        init() {
            if (! this.selectedChannelId) {
                const channelMeta = document.querySelector('[data-current-channel-id]');
                if (channelMeta) {
                    const id = parseInt(channelMeta.getAttribute('data-current-channel-id'), 10);
                    const name = channelMeta.getAttribute('data-current-channel-name');
                    if (id && name) {
                        this.selectedChannelId = id;
                        this.selectedChannelName = name;
                        this.initialChannelId = id;
                        this.initialChannelName = name;
                    }
                }
            }
            if (this.selectedChannelId && ! this.selectedChannelName && this.channels) {
                ...
                if (found) {
                    ...
                    if (! this.initialChannelName && this.initialChannelId === this.selectedChannelId) {
                        this.initialChannelName = found.name;
                    }
                }
            }
            ...
            Livewire.on('question.created', () => {
                if (this.initialChannelId) {
                    this.select({ id: this.initialChannelId, name: this.initialChannelName });
                    return;
                }
                const channelMeta = document.querySelector('[data-current-channel-id]');
                if (channelMeta) {
        ...
        <div class="max-h-48 overflow-y-auto py-1">
            <template x-for="channel in filteredChannels" :key="channel.id">
                <button
                    ...
                    :class="{
                        'font-semibold text-pink-600 dark:text-pink-400': channel.id === selectedChannelId,
                    }"
                >
                    <span x-text="channel.name" class="truncate text-slate-800 dark:text-slate-200"></span>
            ...
            >
                ...
                <span x-show="search.trim()" style="display: none">{{ __('Min 2 characters required') }}</span>
            </div>
        </div>

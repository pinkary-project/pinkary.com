    ...
    <div class="flex items-center gap-3">
        ...
        <div class="overflow-hidden text-sm">
            <div class="flex items-center">
                <p class="truncate font-medium text-black dark:text-white">{{ $result->payload['name'] }}</p>

                @if ($result->payload['isCompanyVerified'])
                    <x-icons.verified-company color="pink-500" class="ml-1 h-3.5 w-3.5" />
                @elseif ($result->payload['isVerified'])
                    <x-icons.verified color="pink-500" class="ml-1 h-3.5 w-3.5" />
                @endif
            </div>

            <p class="truncate text-slate-500">{{ $result->payload['username'] }}</p>
            @if ($result->payload['isFollowedByUser'])
                <div class="flex items-center truncate text-slate-500">
                    <x-icons.user class="mr-1 h-3 w-3" stroke-width="2.5" />
                    Following
                </div>
            @endif
        </div>

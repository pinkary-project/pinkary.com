<div class="flex justify-between border-b border-gray-800 sm:space-x-2 sm:border-b-0 sm:mb-8">
    <a
        href="{{ route('home.feed') }}"
        class="{{ request()->routeIs('home.feed') ? 'bg-pink-600 text-slate-100' : 'text-slate-500 hover:text-slate-100 bg-slate-900 ' }} inline-flex flex-1 items-center justify-center whitespace-nowrap sm:rounded-md border border-transparent px-3 py-3 sm:py-2 text-sm font-medium leading-4 transition duration-150 ease-in-out focus:outline-none"
        title="{{ __('Feed') }}"
        wire:navigate
        wire:transition
    >
        <x-heroicon-o-home
            class="w-6 h-6 xsm:mr-2" />
    ...
    <a
        ...
        wire:transition
    >
        <x-heroicon-o-magnifying-glass class="w-6 h-6 xsm:mr-2" />

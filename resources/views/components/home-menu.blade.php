...
<div class="overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
    <div class="inline-flex min-w-full items-center gap-2 sm:min-w-0">
        @foreach ($tabs as $tab)
            <a
                data-pan="home-tabs-{{ str($tab['label'])->lower() }}"
                href="{{ route($tab['route']) }}"
                class="{{ $tab['active'] ? 'rounded-full bg-[#24314a] px-5 py-2.5 text-white' : 'px-3 py-2.5 text-slate-400 hover:text-white' }} inline-flex items-center justify-center whitespace-nowrap text-sm font-medium leading-4 transition duration-150 ease-in-out focus:outline-none"
                title="{{ $tab['label'] }}"
                wire:navigate
                wire:transition
            >

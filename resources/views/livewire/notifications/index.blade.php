<div class="mb-20 flex flex-col gap-2">
    @foreach ($notifications as $notification)
        @php
            ...
            $isMention = $notification->type === \App\Notifications\UserMentioned::class;
            $isQuestionAnswered = $notification->type === \App\Notifications\QuestionAnswered::class;
        ...
        >
            <div
                ...
                @if (! $isMention)
                    @if ($question->parent_id !== null)
                        <div class="flex items center gap-3 text-sm text-slate-500">
                            <figure
                                class="{{ $question->from->is_company_verified ? 'rounded-md' : 'rounded-full' }} h-10 w-10 flex-shrink-0 bg-slate-800 transition-opacity group-hover:opacity-90">
                                <img
                    ...
                    @elseif ($isQuestionAnswered && $question->answer !== null && $question->from->is(auth()->user()))
                        <div class="flex items-center gap-3 text-sm text-slate-500">
                            <figure
                                class="{{ $question->to->is_company_verified ? 'rounded-md' : 'rounded-full' }} h-10 w-10 flex-shrink-0 bg-slate-800 transition-opacity group-hover:opacity-90">
                                <img
                            ...
                            <p>{{ $question->to->name }} answered your {{ $question->anonymously ? 'anonymous' : '' }}
                                question:</p>
                        </div>
                    @else
                ...
                @elseif ($question->parent !== null)
                    <p class="text-sm text-slate-500">You have been mentioned in a comment
                        by {{ '@' . $question->to->username }}</p>
                @else
                    <p class="text-sm text-slate-500">You have been mentioned in
                        a {{ $question->isSharedUpdate() ? 'update by @'.$question->to->username : 'question:'}}</p>
                @endif

<x-modal name="following" maxWidth="2xl">
    <div class="p-4 md:p-10" x-on:open-modal.window="$event.detail == 'following' ? $wire.set('isOpened', true) : null">
        ...
        @if ($following->isNotEmpty())
                    ...
                    @foreach ($following as $followingUser)
                        ...
                        >
                            <div class="group bg-opacity-80 flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-100 p-4 transition-colors hover:bg-slate-200 dark:border-slate-900 dark:bg-slate-950 dark:hover:bg-slate-900">
                                <figure class="{{ $followingUser->is_company_verified ? 'rounded-md' : 'rounded-full' }} h-12 w-12 shrink-0 overflow-hidden bg-slate-800 transition-opacity group-hover:opacity-90">
                                    <img
                                ...
                                <div class="flex flex-col overflow-hidden text-left text-sm">
                                    <a
                                        ...
                                        x-ref="parentLink"
                                    >
                                        <p class="truncate font-medium text-wrap">{{ $followingUser->name }}</p>
            ...
            <div class="mt-5">{{ $following->links() }}</div>
        @endif
    </div>
</x-modal>

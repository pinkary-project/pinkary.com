    ...
    <section class="mb-12 min-h-screen space-y-10">
        @forelse ($questions as $question)
            <div wire:key="thread-{{ $question->id }}">
                ...
                @else
                    <x-thread :rootId="$question->root?->id" :grandParentId="$question->parent?->parent_id" :parentId="$question->parent?->id" :questionId="$question->id"
                        :username="$question->root?->to->username" />
                @endif
            </div>
        @empty
            <div class="text-center  text-slate-600">There are no questions to show.</div>
        @endforelse

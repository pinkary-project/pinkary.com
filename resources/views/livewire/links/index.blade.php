...
>
    <div class="relative bg-gradient-to-r p-5 text-center text-black dark:text-white">
        <div class="absolute left-0 top-6 flex">
            <x-dropdown-link-profile>
                <x-slot name="trigger">
                    <button
                        ...
                        class="mr-2 flex size-10 items-center justify-center rounded-lg text-slate-600 transition duration-150 ease-in-out dark:text-slate-300"
                    >
                ...
                <x-slot name="content">
                    <button
                        ...
                        type="button"
                        class="mr-2 flex size-10 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-600 transition duration-150 ease-in-out hover:bg-slate-100 hover:text-black dark:border-transparent dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
                    >
            ...
            @if (auth()->user()?->is($user))
                <button
                    class="flex size-10 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-600 transition duration-150 ease-in-out hover:bg-slate-100 hover:text-black dark:border-transparent dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
            ...
            <div class="absolute right-0 top-6 flex">
                @if ($user->followers()->where('follower_id', auth()->id())->exists())
                    <button
                        ...
                        class="flex items-center justify-center rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-slate-600 transition duration-150 ease-in-out hover:bg-slate-200 hover:text-black dark:border-transparent dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
                    >
                        Following
                    </button>
                @else
        ...
        <div class="relative mx-auto h-24 w-24" x-data="{ showAvatar: false }">
            ...
            <div
                ...
                class="fixed inset-0 z-50 flex items-center justify-center bg-slate-50 bg-opacity-75 dark:bg-slate-900"
            >
                <img
                    src="{{ $user->avatar_url }}"
                    alt="{{ $user->username }}"
                    class="size-48 rounded-lg sm:size-64 md:size-80"
                />
            </div>

            @if (auth()->user()?->is($user))
                <button
                    class="absolute right-0 top-0 m-0.5 rounded-md border border-slate-200 bg-slate-50 p-0.5 text-slate-500 transition duration-150 ease-in-out hover:bg-slate-100 hover:text-black dark:border-transparent dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
        ...
        <div class="items center mt-2 flex items-center justify-center">
        ...
        @elseif (auth()->user()?->is($user))
            <a href="{{ route('profile.edit') }}" class="text-sm text-slate-500 hover:underline" wire:navigate>
        ...
        @else
            @if (auth()->user()?->is($user))
                ...
                >
                    @foreach ($links as $link)
                        <li
                            class="hover:darken-gradient {{ $link->is_visible ? 'bg-gradient-to-r' : 'bg-gray-500' }} group relative flex h-12 overflow-hidden"
                        ...
                        >
                            <div
                                x-sortable-handle
                                class="absolute bottom-0 left-0 top-0 z-10 flex w-11 cursor-move items-center justify-center text-slate-50 opacity-50 transition-all duration-500 hover:opacity-100 focus:outline-none group-hover:left-0 sm:-left-10"
                            >
                                <x-heroicon-o-bars-3 class="size-6 opacity-100 group-hover:opacity-100 sm:opacity-0" />
                            </div>

                            <div
                                class="flex flex-grow items-center justify-center transition-all duration-500"
                            ...
                            <div
                                ...
                                class="absolute bottom-0 right-0 top-0 z-10 flex w-11 cursor-pointer items-center justify-center text-slate-50 opacity-50 transition-all duration-500 hover:opacity-100 focus:outline-none group-hover:right-0 sm:-right-10"
                            >
                            ...
                            <div
                                class="z-5 absolute -right-56 bottom-0 top-0 flex items-center justify-center transition-all duration-500"
            ...
            @else
                <div class="space-y-3">
                    @foreach ($links as $link)
                        <div
                            class="{{ $user->link_shape }} {{ $user->gradient }} hover:darken-gradient flex h-12 justify-center bg-gradient-to-r"

<div
    ...
    <div class="relative bg-gradient-to-r p-5 text-center dark:text-white text-black">
        ...
        <div class="relative mx-auto h-24 w-24" x-data="{ showAvatar: false }">
            ...
            @if (auth()->user()?->is($user))
                <button
                    class="absolute right-0 top-0 m-0.5 rounded-fullp-2 border dark:border-transparent border-slate-200 dark:bg-slate-900 bg-slate-50 dark:text-slate-300 text-slate-500 transition duration-150 ease-in-out dark:hover:bg-slate-800 hover:bg-slate-100 dark:hover:text-white hover:text-black"
                    href="{{ route('profile.edit') }}" wire:navigate title="Edit Profile">
        ...
        @elseif (auth()->user()?->is($user))
            <a href="{{ route('profile.edit') }}" class="text-sm text-slate-500 hover:underline" wire:navigate>Tell
                people about yourself</a>
        @endif
    ...
    <div class="py-5">
        ...
        @else
            @if (auth()->user()?->is($user))
                <ul x-data="{ isDragging: false }" x-sortable x-on:choose.stop="isDragging = true"
                    ...
                    @foreach ($links as $link)
                        <li class="relative h-12 hover:darken-gradient group flex {{ $link->is_visible ? 'bg-gradient-to-r' : 'bg-gray-500' }} overflow-hidden shadow-md"
            ...
            @else
                <div class="space-y-3">
                    @foreach ($links as $link)
                        <div class="{{ $user->link_shape }} {{ $user->gradient }} h-12 hover:darken-gradient flex justify-center bg-gradient-to-r shadow-md"
        ...
        <div x-data="{
            ...
            <div>
                <div class="flex gap-2">
                    <button x-on:click="showLinksForm = ! showLinksForm ; showSettingsForm = false"
                ...
                <div x-show="showLinksForm" x-transition:enter="transition duration-300 ease-out"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                    x-transition:leave="transition duration-300 ease-in" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0" class="mt-4" x-cloak>

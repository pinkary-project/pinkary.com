<div @if (auth()->user()?->is($user)) x-data="{
    ...
    @if (auth()->check() && (auth()->user()->isBlocking($user) || auth()->user()->isBlockedBy($user)))
        <div class="flex flex-col items-center justify-center py-20 text-center">
            <div class="mb-4 rounded-full bg-slate-200 p-6 dark:bg-slate-800">
                <x-heroicon-o-no-symbol class="size-12 text-slate-400" />
            </div>
            <h2 class="text-2xl font-bold">User Blocked</h2>
            <p class="mt-2 text-slate-500">You cannot view this profile because of a block.</p>
            @if (auth()->user()->isBlocking($user))
                <button
                    type="button"
                    wire:click="unblock({{ $user->id }})"
                    class="mt-6 rounded-lg bg-red-600 px-4 py-2 font-bold text-white hover:bg-red-700"
                >
    ...
    @else
        <div class="relative bg-gradient-to-r p-5 text-center dark:text-white text-black">
            ...
            @if (! $user->is(auth()->user()))
                <div class="absolute right-0 top-6 flex space-x-2">
                    @auth
                        @if (auth()->user()->blocks()->where('blocked_id', $user->id)->exists())
                            <button
                                type="button"
                                wire:click="unblock({{ $user->id }})"
                                class="flex items-center justify-center rounded-lg border dark:border-transparent border-slate-200 bg-red-600 px-2 py-1 text-white transition duration-150 ease-in-out hover:bg-red-700"
                            >
                                Unblock
                            </button>
                        @else
                            ...
                            <x-dropdown align="right" width="48">
                                <x-slot name="trigger">
                                    <button class="flex items-center justify-center rounded-lg border dark:border-transparent border-slate-200 dark:bg-slate-900 bg-slate-50 p-2 dark:text-slate-300 text-slate-600 transition duration-150 ease-in-out dark:hover:bg-slate-800 hover:bg-slate-200 active:bg-slate-100 focus:outline-none">
                                        <x-heroicon-o-ellipsis-horizontal class="size-5" />
                                    </button>
                                </x-slot>
                                <x-slot name="content">
                                    <x-dropdown-button wire:click="block({{ $user->id }})" class="text-red-600 dark:text-red-400">
                                        {{ __('Block User') }}
                                    </x-dropdown-button>
                                </x-slot>
                            </x-dropdown>

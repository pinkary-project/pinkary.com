<article class="block space-y-3" id="q-{{ $questionId }}" x-data="copyCode">
        ...
        <div class="flex items-start {{ $question->isSharedUpdate() ? 'justify-end' : 'justify-between gap-3' }}">
            @unless ($question->isSharedUpdate())
                @if ($question->anonymously)
                    <div class="inline-flex items-center gap-3 border border-dashed border-slate-800/40 bg-[#0b1324] px-3 py-2 text-sm text-slate-400">
                ...
                <div class="flex items-center gap-2 self-start">
                    <a
                        class="inline-flex items-center gap-1.5 border border-slate-800/40 bg-[#111a2d] px-2.5 py-1.5 text-[0.72rem] font-medium text-slate-400 transition hover:bg-[#1a2440] hover:text-white"
                    ...
                    @if ($question->pinned && $pinnable)
                        <div class="inline-flex items-center gap-1.5 border border-slate-800/40 bg-[#0b1324] px-2.5 py-1.5 text-[0.72rem] font-medium text-slate-400">
    ...
    @if ($question->answer)
        ...
        <div
            ...
            class="group px-0 py-0.5 {{ $previousQuestionId === $questionId ? 'bg-[#151225]' : '' }}
        ...
        >
            <div class="flex items-start justify-between gap-3">
                <a
                    ...
                    wire:navigate
                >
                    <figure class="{{ $question->to->is_company_verified ? 'rounded-2xl' : 'rounded-full' }} h-12 w-12 flex-shrink-0 border border-slate-800/40 bg-[#10182b] transition-opacity group-hover/profile:opacity-90">
                        <img
                ...
                @if (auth()->check() && auth()->user()->can('update', $question))
                    ...
                    >
                        <x-slot name="trigger">
                            <button
                                data-navigate-ignore="true"
                                class="inline-flex items-center border border-slate-800/40 bg-[#0b1324] px-2.5 py-1.5 text-sm text-slate-400 transition duration-150 ease-in-out hover:bg-[#1a2440] hover:text-white focus:outline-none">
            ...
            @if((! $inThread || $commenting) && $question->parent)
                <a href="{{
                   ...
                   data-navigate-ignore="true"
                   wire:navigate
                         class="mt-3 inline-flex max-w-full items-center gap-2 border border-slate-800/40 bg-[#0b1324] px-3 py-1.5 text-xs font-medium text-slate-400 transition-colors hover:bg-[#1a2440] hover:text-white"
                >
            ...
            <div class="mt-3 flex flex-col gap-2 border-t border-slate-800/40 pt-2.5 sm:flex-row sm:items-center sm:justify-between">

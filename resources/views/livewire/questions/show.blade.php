<article class="block" id="q-{{ $questionId }}" x-data="copyCode">
    <div>
        <div class="{{ $question->isSharedUpdate() ? "justify-end" : "justify-between" }} flex">
            @unless ($question->isSharedUpdate())
                @if ($question->anonymously)
                    <div class="flex items-center gap-3 px-4 text-sm text-slate-500">
                        <div
                            class="border-1 flex h-10 w-10 items-center justify-center rounded-full border border-dashed border-slate-600 dark:border-slate-400"
                        >
                            <span>?</span>
                        </div>
    ...
    @if ($question->answer)
        <div
            ...
            class="{{ $previousQuestionId === $questionId ? "bg-slate-200/60 dark:bg-slate-700/60" : "border border-slate-200 bg-slate-50 dark:border-transparent dark:bg-slate-900" }} {{ $commenting ?: "cursor-pointer transition-colors duration-100 ease-in-out hover:bg-slate-100/60 dark:hover:bg-slate-700/60" }} group mt-3 rounded-2xl p-4"
        >
            <div class="flex justify-between">
                ...
                >
                    <figure
                        class="{{ $question->to->is_company_verified ? "rounded-md" : "rounded-full" }} h-10 w-10 flex-shrink-0 bg-slate-100 transition-opacity group-hover/profile:opacity-90 dark:bg-slate-800"
                    >
                        <img
                    ...
                    <div class="overflow-hidden text-sm">
                        ...
                        <p
                            class="truncate text-slate-500 transition-colors group-hover/profile:text-slate-600 dark:group-hover/profile:text-slate-400"
                        >
            ...
            <div x-data="showMore">
                <div
                    class="answer mt-3 overflow-hidden break-words text-slate-800 dark:text-slate-200"
                    wire:ignore.self
                    x-ref="parentDiv"
                >
                ...
                <div x-show="showMore === true" class="answer mt-1">
                    <button
                        data-navigate-ignore="true"
            ...
            <div class="mt-3 flex items-center justify-between text-sm text-slate-500">
                <div class="flex items-center gap-1">
                    <a
                        ...
                        @class([
                            "flex items-center transition-colors hover:text-slate-600 focus:outline-none group-hover:text-pink-500 dark:hover:text-slate-400",
                ...
                <div class="flex items-center text-slate-500">
                    ...
                    <button
                        ...
                        class="mr-1 flex items-center transition-colors hover:text-slate-600 focus:outline-none dark:hover:text-slate-400"
                    >
                            ...
                            <button
                                ...
                                x-on:click="
                                    twitter({
                                        ...
                                        question:
                                            '{{ str_replace("'", "\'", $question->isSharedUpdate() ? $question->answer : $question->content) }}',
                                        message:
                                            '{{ $question->isSharedUpdate() ? "See it on Pinkary" : "See response on Pinkary" }}',
                                    })
                                "
                                type="button"
                                class="text-slate-500 transition-colors hover:text-slate-600 focus:outline-none dark:hover:text-slate-400"

<article class="block" id="q-{{ $questionId }}" x-data="copyCode">
        ...
        <div class="flex {{ $question->isSharedUpdate() ? 'justify-end' : 'justify-between' }}">
            @unless ($question->isSharedUpdate())
                @if ($question->anonymously)
                    <div class="flex items-center gap-3 px-4 text-sm text-slate-500">
                        <div
                            class="border-1 flex h-10 w-10 items-center justify-center rounded-full border border-dashed  border-slate-600">
                            <span>?</span>
                        </div>
                ...
                @endif
                <a class="text-xs text-pink-500 flex items-end gap-1.5"
                    href="{{ 'https://translate.google.com/?sl=auto&tl=en&text=' . urlencode($question->sharable_content) }}"
                    target="_blank" data-navigate-ignore="true">
            ...
            @if ($question->pinned && $pinnable)
                <div class="mb-2 flex items-center space-x-1 px-4 text-sm focus:outline-none">
                    <x-icons.pin class="h-4 w-4  text-slate-600" />
                    <span class=" text-slate-600">Pinned</span>
                </div>
            @endif
        </div>

        @unless ($question->isSharedUpdate())
            <p class="mt-3 px-4  text-slate-800">
                {!! $question->content !!}
            </p>
        @endunless
    </div>
    ...
    @if ($question->answer)
        <div data-parent=true x-intersect.once.full="$dispatch('post-viewed', { postId: '{{ $questionId }}' })"
            x-data="clickHandler" x-on:click="handleNavigation($event)"
            class="group p-4 mt-3 rounded-2xl {{ $previousQuestionId === $questionId ? ' bg-slate-200/60' : 'border  border-slate-200  bg-slate-50' }}
            {{ $commenting ?: 'cursor-pointer transition-colors duration-100 ease-in-out  hover:bg-slate-100/60' }}">
            <div class="flex justify-between">
                <a href="{{ route('profile.show', ['username' => $question->to->username]) }}"
                    class="group/profile flex items-center gap-3" data-navigate-ignore="true" wire:navigate>
                    <figure
                        class="{{ $question->to->is_company_verified ? 'rounded-md' : 'rounded-full' }} h-10 w-10 flex-shrink-0  bg-slate-100 transition-opacity group-hover/profile:opacity-90">
                        <img src="{{ $question->to->avatar_url }}" alt="{{ $question->to->username }}"
                    ...
                    <div class="overflow-hidden text-sm">
                        ...
                        <p class="truncate text-slate-500 transition-colors  group-hover/profile:text-slate-600">
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button data-navigate-ignore="true"
                                class="inline-flex items-center rounded-md border border-transparent py-1 text-sm  text-slate-600 transition duration-150 ease-in-out  hover:text-slate-950 focus:outline-none">
                        ...
                        <x-slot name="content">
                            @if (!$question->pinned && auth()->user()->can('pin', $question))
                                <x-dropdown-button data-navigate-ignore="true" wire:click="pin"
                                    class="flex items-center gap-1.5">
                                    <x-icons.pin class="h-4 w-4" />
                                    <span>Pin</span>
                                </x-dropdown-button>
                            @elseif ($question->pinned)
                                <x-dropdown-button data-navigate-ignore="true" wire:click="unpin"
                                    class="flex items-center gap-1.5">
                                    <x-icons.pin class="h-4 w-4" />
                                    <span>Unpin</span>
                                </x-dropdown-button>
                            ...
                            @if (
                                !$question->is_ignored &&
                                    $question->answer_created_at?->diffInHours() < 24 &&
                                    auth()->user()->can('update', $question))
                                <x-dropdown-button data-navigate-ignore="true"
            ...
            @if ((!$inThread || $commenting) && $question->parent)
                <a href="{{ route('questions.show', [
                    ...
                    data-navigate-ignore="true" wire:navigate
                    class="truncate text-xs text-slate-500 transition-colors  hover:text-slate-600">
                    In response to {{ '@' . $question->parent->to->username }}
                </a>
            @endif

            <div x-data="showMore">
                <div class="mt-3 break-words  text-slate-800 overflow-hidden answer" wire:ignore.self x-ref="parentDiv">
                ...
                <div x-show="showMore === true" class="mt-1 answer">
                    <button data-navigate-ignore="true" @click="showButtonAction"
                        class="text-sm text-pink-500 flex ml-auto" x-text="showMoreButtonText"></button>
                </div>
            </div>

            <div class="mt-3 flex items-center justify-between text-sm text-slate-500">
                <div class="flex items-center gap-1">
                    <a @if (!$commenting) x-ref="parentLink"
                        ...
                        @class([
                            'flex items-center transition-colors group-hover:text-pink-500  hover:text-slate-600 focus:outline-none',
                    ...
                    <button x-data="likeButton('{{ $question->id }}', @js(auth()->check()))" x-cloak data-is-liked="@js($likeExists)"
                        data-likes-count="{{ $likesCount }}" data-navigate-ignore="true" x-on:click="toggleLike"
                        :title="likeButtonTitle"
                        class="flex items-center transition-colors  hover:text-slate-600 focus:outline-none">
                    ...
                    <a class="flex items-center transition-colors group-hover:text-pink-500  hover:text-slate-600 focus:outline-none cursor-pointer"
                        title="Translate"
                ...
                <div class="flex items-center text-slate-500 ">
                    ...
                    <button data-navigate-ignore="true" x-data="bookmarkButton('{{ $question->id }}', @js(auth()->check()))"
                        data-is-bookmarked="@js($question->is_bookmarked)"
                        data-bookmarks-count="{{ $question->bookmarks_count }}" x-cloak x-on:click="toggleBookmark"
                        :title="bookmarkButtonTitle"
                        class="mr-1 flex items-center transition-colors  hover:text-slate-600 focus:outline-none">
                        ...
                        <x-slot name="trigger">
                            <button data-navigate-ignore="true"
                                x-bind:class="{
                                    ...
                                    'text-slate-500  hover:text-slate-600': !open
                                }"
                                title="Share"
        ...
        @if (
            ...
            <x-modal max-width="md" name="question.edit.answer.{{ $questionId }}">
                <div class="p-8">
                    <h2 class="text-lg font-medium  text-slate-950">Edit</h2>
        ...
        <x-modal max-width="md" name="question.delete.{{ $questionId }}.confirmation">
            <div class="p-8">
                <h2 class="text-lg font-medium  text-slate-950">Delete Question</h2>
                ...
                <div class="mt-4 flex items-center justify-between">
                    ...
                    <x-primary-button wire:click="ignore">
                        Delete
                    </x-primary-button>
                </div>
            </div>

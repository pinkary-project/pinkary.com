<div class="mb-12 pt-4" id="questions-create">
    <form wire:submit="store" wire:keydown.cmd.enter="store" wire:keydown.ctrl.enter="store" x-data="imageUpload"
        x-init='() => {
        ...
        <div class="relative group/menu">
            <div x-data="{ content: $persist($wire.entangle('content')).as('{{ $this->draftKey }}') }">
                <x-textarea x-model="content" placeholder="{{ $this->placeholder }}"
                    maxlength="{{ $this->maxContentLength }}" rows="3" required x-autosize x-ref="content"
                    autocomplete />
            </div>
                ...
                <template x-for="(image, index) in images" :key="index">
                    <div class="relative h-20 w-20">
                        ...
                        <button @click="removeImage($event, index)"
                            class="absolute top-0.5 right-0.5 p-1 rounded-md  bg-slate-200 bg-opacity-75  text-slate-600 hover:text-pink-500">
            ...
            <p class="text-right text-xs  text-slate-600"><span x-text="$wire.content.length"></span>
                / {{ $this->maxContentLength }}</p>
        ...
        <div class="mt-4 flex items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <button title="Upload an image" x-ref="imageButton"
                    ...
                    class="p-1.5 rounded-lg border  border-slate-200  bg-slate-50 text-sm  text-slate-600 hover:text-pink-500  hover:bg-slate-100"
